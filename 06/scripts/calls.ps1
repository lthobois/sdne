param([string]$BaseUrl = "http://localhost:5062")

function Api($Method, $Path, $Body = $null) {
  Write-Host "`n=== $Method $Path ==="
  if ($Body -ne $null) { Invoke-RestMethod -Method $Method -Uri "$BaseUrl$Path" -ContentType "application/json" -Body ($Body | ConvertTo-Json) }
  else { Invoke-RestMethod -Method $Method -Uri "$BaseUrl$Path" }
}
